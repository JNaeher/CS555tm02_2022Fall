...
class TestUS35(unittest.TestCase):
    def test_recent_births(self):
        data = organize('test_file.ged')
        self.assertEqual(recent_births_and_deaths(data)[0], [{'ID': '@I24@','age': 0,'alive': True,'birthday': '19 APR 2023','child': '@F7@','death': None,'gender': 'F','name': 'Stormi /Webster/','spouse': None}])

class TestUS36(unittest.TestCase):
    def test_recent_deaths(self):
        data = organize('test_file.ged')
        self.assertEqual(recent_births_and_deaths(data)[1], [{'ID': '@I5@','age': 8,'alive': False,'birthday': '14 DEC 2014','child': '@F1@','death': '19 APR 2023','gender': 'F','name': 'Reign /Disick/','spouse': None}])
...
class TestUS20(unittest.TestCase):
    def test_aunts_uncles_not_marry_neices_nephews(self):
        data = organize('test_file.ged')
        self.assertEqual(aunts_uncles_not_marry_niece_nephews(data), True) 

class TestUS38(unittest.TestCase):
    def test_upcoming_births(self):
        data = organize('test_file.ged')
        self.assertEqual(len(upcoming_birthdays(data)), 3) 
